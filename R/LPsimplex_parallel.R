#' LPsimplex_parallel
#'
#' This function solves a linear programming problem using the simplex method with parallelization.
#'
...
#' @param max A logical value indicating whether to maximize the objective function.
...
#' b=c(360,300,200)
#' slack=c(3,4,5)
#' result <- LPsimplex_parallel(c, A, b, slack, max = TRUE, process = TRUE)
#' print(result)
#' @export
LPsimplex_parallel <- function(c, A, b, slack, max = TRUE, process = TRUE) {
  if (!max) c <- -c
  ...
  if (process) print_simplex_table(c[B], B, b, A, sigma, "初始单纯形表")
  unbounded = FALSE
  while (any(sigma > 0)) {
    infty <- which(sigma > 0)
    ...
    # 调用并行化函数
    result <- parallel_simplex_step(c, A, b, B, pcol, prow, process)
    A <- result$A
    b <- result$b
    sigma <- result$sigma
    B <- result$B
  ...
  if (process) print_simplex_table(c[B], B, b, A, sigma, "最终单纯形表")
